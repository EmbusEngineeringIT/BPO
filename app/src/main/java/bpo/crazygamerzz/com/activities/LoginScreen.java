...
public class LoginScreen extends AppCompatActivity implements View.OnClickListener{
    ...
    private SharedPreferences sharedPreferences;
    private SharedPreferences.Editor editor;
    public static String emailKey="emailKey",usernameKey = "username";
    private BroadcastReceiver broadcastReceiver;
    public static String APP_PREFERENCES="BPOLOCALEMPID";
    public static boolean LOGIN_CONDITION=false;
    public static String LOGIN_SHAR="condition";
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        getWindow().setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_STATE_HIDDEN);
            ...
            loginService.checkLogin(email, password).enqueue(new Callback<LoginData>()
            {
                ...
                public void onResponse(Call<LoginData> call, Response<LoginData> response) {
                    ...
                    if(response.isSuccessful())
                    {
                        ...
                        startActivity(intent);
                        editor=sharedPreferences.edit();
                        editor.putBoolean(LOGIN_SHAR,LOGIN_CONDITION);
                        editor.putString(emailKey,response.body().getEmail());
                        editor.putString(usernameKey,response.body().getUserName());
                        editor.putString("LOGIN_STATUS","1");
                        editor.commit();
                        finish();
                        Log.i("fial", "post submitted to API." + response.body().toString());
                        ...
                        {
                            //Toast.makeText(LoginScreen.this,"else success!",Toast.LENGTH_LONG).show();
                            try
                        {
                            loginDialog.dismiss();
                            String errorResponse = new String(response.errorBody().bytes());
                ...
                {
                    loginDialog.dismiss();
                    Toast.makeText(LoginScreen.this,"Unable to submit post to API.",Toast.LENGTH_LONG).show();
                    Log.e("", "Unable to submit post to API."+t.getMessage());
                }
            });
        }
